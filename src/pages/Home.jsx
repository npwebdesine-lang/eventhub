...
import { Camera, Heart, Loader2, PartyPopper, MapPin, X, RefreshCw, Zap, Users, Car, Info, ChevronLeft, ChevronRight } from 'lucide-react';
import { supabase } from '../lib/supabase';
import gsap from 'gsap';

const MODULES_INFO = {
  photo: { title: 'כל אחד צלם', description: 'העלו תמונות לאלבום המשותף של האירוע', icon: Camera, color: 'text-orange-500', bg: 'bg-orange-50' },
  dating: { title: 'דייט-ליין', description: 'הרשת החברתית של האירוע לרווקים ורווקות', icon: Heart, color: 'text-rose-500', bg: 'bg-rose-50' },
  icebreaker: { title: 'שובר קרח', description: 'משחק משימות חברתי עם אורחים אחרים', icon: Zap, color: 'text-cyan-500', bg: 'bg-cyan-50' },
  rideshare: { title: 'לוח טרמפים', description: 'שתפו נסיעות לאחר האירוע', icon: Car, color: 'text-amber-500', bg: 'bg-amber-50' },
  seating: { title: 'סידור הושבה', description: 'מצאו את השולחן שלכם ואת בני השולחן', icon: MapPin, color: 'text-emerald-500', bg: 'bg-emerald-50' }
};
...
const Home = () => {
    ...
    const fetchEvent = async () => {
      try {
        const { data, error } = await supabase
          .from('events')
          .select('id, name, active_modules, design_config, event_date, location')
          .eq('id', id)
          .single();
  ...
  useEffect(() => {
    ...
    const fetchMyTable = async () => {
      ...
      try {
        ...
        if (error) throw error;
        if (isMounted) {
          setMyTable(data?.length > 0 ? { found: true, number: data[0].table_number } : { found: false });
        }
      } catch {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center" dir="rtl">
  ...
  if (!eventData) return <div className="min-h-screen flex items-center justify-center text-slate-500" dir="rtl">לא נמצא אירוע.</div>;
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 transition-colors duration-1000" style={{ backgroundColor: background }} dir="rtl">
        <div className="bg-white p-8 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.10)] w-full max-w-sm text-center animate-in zoom-in duration-500">
          ...
          <h1 className="text-3xl font-black text-slate-800 mb-1">ברוכים הבאים!</h1>
          <p className="text-slate-500 mb-8 font-medium text-sm">
            ל-<span className="font-bold text-slate-700">{name}</span><br />
  ...
  const CustomModuleCard = ({ mKey, onClick, hasBadge }) => {
    ...
    return (
      ...
      >
        ...
        {/* Icon */}
        <div className={`w-12 h-12 ${info.bg} rounded-[1rem] flex items-center justify-center mb-3 mt-1`}>
          <info.icon size={24} className={info.color} />
        </div>

        {/* Title + description */}
        <h3 className="font-bold text-slate-800 text-sm leading-tight">{info.title}</h3>
        <p className="text-slate-400 text-xs mt-1 leading-relaxed line-clamp-2">{info.description}</p>

        {/* Arrow hint */}
        <ChevronLeft size={13} className="absolute bottom-4 left-4 text-slate-200 group-hover:text-slate-400 transition-colors" />
      </div>
    );
  };
  ...
  // --- Main Event Screen ---
  return (
    <div className="min-h-screen flex flex-col font-sans pb-12 transition-colors duration-1000" style={{ backgroundColor: background }} dir="rtl">
      ...
      >
        <div className="max-w-md w-full header-anim">
          <p className="text-white/60 font-bold text-xs uppercase tracking-widest mb-1">{getGreeting()}</p>
          <h1 className="text-3xl md:text-4xl font-black text-white mb-1 leading-tight drop-shadow-sm">{name}</h1>
          {eventData.event_date && (
            <p className="text-white/60 text-xs font-bold mb-5">
              {new Date(eventData.event_date).toLocaleDateString('he-IL', { weekday: 'long', day: 'numeric', month: 'long' })}
          ...
          <div className="bg-black/20 rounded-[1.5rem] p-4 flex items-center justify-between border border-white/10 backdrop-blur-sm">
            <div className="flex flex-col text-right">
              ...
              <span className="text-white font-bold text-sm">{guestNameStr}</span>
            </div>
            <button
      ...
      <div className="px-5 -mt-10 relative z-20 w-full max-w-md mx-auto flex-1 flex flex-col gap-4">
        ...
        {active_modules.seating && (
          <div className="module-card-anim relative bg-white rounded-[2rem] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.07)] border border-slate-100 flex flex-col items-center text-center">
            <button onClick={(e) => openInfo(e, 'seating')} className="absolute top-5 left-5 text-slate-200 hover:text-slate-400 z-10 p-1 transition-colors">
              <Info size={18} />
            </button>
            <div
            ...
            {myTable === null ? (
              ...
              <>
                <p className="text-slate-400 font-bold mb-1 text-xs uppercase tracking-wide">השולחן שלך</p>
                <div className="text-7xl font-black mb-5 leading-none" style={{ color: primary }}>{myTable.number}</div>
                <button
              ...
              <div className="py-2">
                ...
                <p className="text-slate-400 text-sm font-medium mb-5">לא מצאנו את השם &quot;{guestNameStr}&quot;.</p>
                <button
                  onClick={handleChangeName}
        ...
        <div className="grid grid-cols-2 gap-3">
          {active_modules.photo && (
            <CustomModuleCard mKey="photo" onClick={() => navigate(`/photos?event=${id}`)} />
          )}
          {active_modules.dating && (
            <CustomModuleCard mKey="dating" onClick={() => navigate(`/dating?event=${id}`)} hasBadge={hasUnreadDating} />
          )}
          {active_modules.icebreaker && (
            <CustomModuleCard mKey="icebreaker" onClick={() => navigate(`/icebreaker?event=${id}`)} />
          )}
          {active_modules.rideshare && (
            <CustomModuleCard mKey="rideshare" onClick={() => navigate(`/rideshare?event=${id}`)} />
          )}
        </div>
      ...
      {infoModal && (
        ...
        >
          ...
          >
            <button onClick={() => setInfoModal(null)} className="absolute top-4 right-4 text-slate-400 bg-slate-50 hover:bg-slate-100 p-2 rounded-full transition-colors">
              <X size={20} />
            </button>
            <div className={`w-20 h-20 mx-auto rounded-[1.5rem] flex items-center justify-center mb-6 ${infoModal.bg}`}>
      ...
      {showMatesModal && (
        ...
        >
          <div
            ...
            onClick={e => e.stopPropagation()}
          >
            <button onClick={() => setShowMatesModal(false)} className="absolute top-4 right-4 text-slate-400 bg-slate-50 hover:bg-slate-100 p-2 rounded-full transition-colors z-10">
              <X size={20} />
            </button>
            <div className="w-20 h-20 rounded-[1.5rem] flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: `${primary}18` }}>
              <Users size={36} style={{ color: primary }} />
            </div>
            <h3 className="text-2xl font-black text-slate-800 mb-1">השותפים לשולחן</h3>
            <p className="text-slate-400 font-bold mb-6 text-sm">שולחן מספר {myTable?.number}</p>
            <div className="overflow-y-auto bg-slate-50 rounded-[1.5rem] p-5 text-right border border-slate-100 flex-1">
              {loadingMates ? (
                ...
                <ul className="space-y-3">
                  {tableMates.map((mate, idx) => (
                    <li key={idx} className="text-slate-700 font-bold flex items-center gap-3">
                      <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: primary }} />
                ...
                <p className="text-slate-400 text-center py-4 font-medium leading-relaxed text-sm">
                  נראה שאת/ה לבד בשולחן הזה כרגע.<br />
