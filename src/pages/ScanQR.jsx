    ...
    >
      ...
      <div className="px-6 -mt-6 relative z-20 w-full max-w-sm mx-auto flex-1 flex flex-col">
            ...
            <div className="p-8 text-center">
              ...
              <form onSubmit={handleManualSubmit}>
                <input
                  ...
                  className="w-full p-4 bg-slate-50 border border-slate-200 rounded-[1.2rem] outline-none font-mono text-center text-xl font-black tracking-widest mb-4 focus:ring-2 focus:ring-blue-500 transition-all"
                  dir="ltr"
                />
                <button
                  type="submit"
                  disabled={processing || !manualCode.trim()}
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-4 rounded-[1.2rem] shadow-lg hover:scale-[1.02] transition-transform active:scale-95 flex justify-center items-center gap-2 disabled:opacity-50"
                >
                  {processing ? (
            ...
            <div className="relative rounded-[2rem] overflow-hidden aspect-[4/5] bg-slate-900 border-4 border-slate-50">
              ...
              <div className="absolute inset-0 pointer-events-none flex items-center justify-center p-8">
                <div className="w-full h-full border-2 border-white/20 rounded-[2rem] relative shadow-[inset_0_0_50px_rgba(0,0,0,0.3)]">
                  <div className="absolute -top-1 -left-1 w-10 h-10 border-t-4 border-l-4 border-blue-500 rounded-tl-[2rem]"></div>
                  <div className="absolute -top-1 -right-1 w-10 h-10 border-t-4 border-r-4 border-blue-500 rounded-tr-[2rem]"></div>
                  <div className="absolute -bottom-1 -left-1 w-10 h-10 border-b-4 border-l-4 border-blue-500 rounded-bl-[2rem]"></div>
                  <div className="absolute -bottom-1 -right-1 w-10 h-10 border-b-4 border-r-4 border-blue-500 rounded-br-[2rem]"></div>
                </div>
              </div>
            </div>
          )}
        ...
        <button
          ...
          className="mt-auto mb-10 w-full py-4 text-white/60 hover:text-white font-bold flex justify-center items-center gap-2 transition-colors"
        >
          {manualMode ? (
            <>
