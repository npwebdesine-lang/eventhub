...
// פונקציה לחישוב בהירות של צבע (Luminance) כדי לדעת אם הרקע בהיר או כהה
...
const Invite = () => {
  ...
  useEffect(() => {
    ...
    if (template === 'modern') {
      gsap.to([bgDecor1.current, bgDecor2.current], {
        x: "random(-60, 60)", y: "random(-60, 60)", scale: "random(0.8, 1.2)",
        duration: "random(4, 8)", repeat: -1, yoyo: true, ease: "sine.inOut"
      });
    } else if (template === 'elegant') {
      gsap.to(".elegant-particle", {
        y: "random(-80, 80)", x: "random(-40, 40)", scale: "random(0.6, 1.4)", opacity: "random(0.1, 0.4)",
        duration: "random(4, 9)", repeat: -1, yoyo: true, ease: "sine.inOut", stagger: { amount: 2, from: "random" }
      });
      gsap.to(".pulse-ring", { scale: 1.05, opacity: 0.4, duration: 2, repeat: -1, yoyo: true, ease: "sine.inOut" });
    } else if (template === 'corporate') {
      gsap.to(bgDecor1.current, { rotation: 360, duration: 60, repeat: -1, ease: "linear" });
    }
  }, [loading, eventData]);

  useEffect(() => {
    if (showRsvp) gsap.fromTo(".step-anim", { opacity: 0, x: 20 }, { opacity: 1, x: 0, duration: 0.4, ease: "power2.out" });
  }, [rsvpStep, showRsvp]);
    ...
    return (
      <div className="fade-up-item mt-8 pt-8 border-t border-opacity-20 space-y-4 relative z-20" style={{ borderColor: isLight ? '#000000' : '#ffffff' }}>
        {active_modules?.rsvp !== false && (
          <button onClick={() => { setShowRsvp(true); setRsvpStep(1); }} className="w-full flex items-center justify-center gap-3 text-white font-black py-4 rounded-[1.5rem] text-lg shadow-xl hover:scale-[1.02] transition-transform" style={{ backgroundColor: primaryColor }}>
            <CheckCircle2 size={24} /> אישור הגעה (RSVP)
          </button>
        )}
        <div className="flex flex-col md:flex-row gap-3">
          {location && (
            <a href={`https://waze.com/ul?q=${encodeURIComponent(location)}&navigate=yes`} target="_blank" rel="noopener noreferrer" className={`flex-1 flex items-center justify-center gap-2 font-bold py-4 rounded-[1.5rem] text-sm transition-all active:scale-95 shadow-sm ${isLight ? 'bg-[#e5f0ff] text-[#007ee5] hover:bg-[#d0e6ff]' : 'bg-[#007ee5]/20 text-[#3399ff] border border-[#007ee5]/30 hover:bg-[#007ee5]/30'}`} title={`ניווט אל: ${location}`}>
              <Navigation size={18} /> נווט לאירוע 
            </a>
          )}
          {active_modules?.rideshare && (
            <button onClick={() => navigate(`/rideshare?event=${id}`)} className={`flex-1 flex items-center justify-center gap-2 font-bold py-4 rounded-[1.5rem] text-sm transition-all active:scale-95 shadow-sm ${isLight ? 'bg-slate-100 text-slate-800 hover:bg-slate-200' : 'bg-white/10 text-white hover:bg-white/20 border border-white/10'}`}>
  ...
  const renderTemplate = () => {
      ...
      return (
        <div className="min-h-screen bg-[#ffffff] flex flex-col items-center p-6 text-center relative overflow-hidden" dir="rtl">
          ...
          <div className="absolute inset-0 pointer-events-none z-0">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="elegant-particle absolute rounded-full opacity-20 blur-[2px]" style={{ backgroundColor: primaryColor, width: `${Math.random() * 20 + 10}px`, height: `${Math.random() * 20 + 10}px`, top: `${[10, 20, 70, 80, 40, 60][i]}%`, left: `${[10, 80, 20, 90, 50, 70][i]}%` }}></div>
            ))}
          </div>
          ...
          <div className="w-full max-w-md mx-auto pt-8 pb-20 relative z-10">
            ...
            <div className="fade-up-item relative w-64 h-64 mx-auto mb-12">
              ...
              {inviteImage ? <img src={inviteImage} className="w-full h-full object-cover rounded-full p-1 relative z-10 shadow-lg bg-white" alt="Event Cover" /> : <div className="w-full h-full rounded-full bg-slate-100 flex items-center justify-center p-1 relative z-10"><CalendarHeart size={48} className="text-slate-300" /></div>}
            </div>
            {!isHappeningNow ? (
      ...
      return (
        <div className="min-h-screen bg-slate-50 flex flex-col items-center p-6 text-center relative overflow-hidden" dir="rtl">
          ...
          <div className="w-full max-w-md mx-auto mt-12 relative z-10 pb-20">
            ...
            <div className="fade-up-item bg-slate-800 rounded-2xl shadow-2xl -mt-8 mx-4 p-6 relative z-20 text-white" style={{ backgroundColor: primaryColor }}>
              {!isHappeningNow ? (
    ...
    const boxBgColor = isLightBg ? 'bg-white/60 border-white/50 text-slate-800 shadow-xl' : 'bg-white/10 border-white/10 text-white shadow-2xl hover:bg-white/20';

    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden text-center transition-colors duration-1000" style={{ backgroundColor: bgColor }} dir="rtl">
        ...
        <div className="relative z-10 w-full max-w-lg mx-auto pb-20">
          <div className="fade-up-item mx-auto w-24 h-24 rounded-[2rem] flex items-center justify-center mb-8 shadow-2xl backdrop-blur-md" style={{ backgroundColor: `${primaryColor}20`, border: `1px solid ${primaryColor}40` }}>
            {inviteImage ? <img src={inviteImage} className="w-full h-full object-cover rounded-[2rem] p-1 shadow-inner bg-white/50" alt="Event Cover" /> : <PartyPopper size={40} style={{ color: primaryColor }} />}
          </div>
          
          <div className={`fade-up-item mb-2 font-medium tracking-widest uppercase text-sm`} style={{ color: isLightBg ? primaryColor : 'rgba(255,255,255,0.7)' }}>Save The Date</div>
          <h1 className={`fade-up-item text-5xl md:text-6xl font-black ${headerTextColor} mb-6 leading-tight drop-shadow-lg`}>{name}</h1>
          <p className={`fade-up-item text-xl ${subTextColor} mb-12 font-medium flex items-center justify-center gap-2`}><Clock size={20} /> {new Date(event_date).toLocaleDateString('he-IL')}</p>
          
          {!isHappeningNow ? (
            <div className="fade-up-item flex justify-center gap-3 md:gap-4 mb-12" dir="ltr">
              <div className="flex flex-col items-center"><div className={`w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center text-2xl md:text-3xl font-black backdrop-blur-lg border transition-colors ${boxBgColor}`}>{timeLeft.seconds.toString().padStart(2, '0')}</div><span className="text-xs font-bold mt-3" style={{ color: isLightBg ? primaryColor : 'white' }}>שניות</span></div>
              <div className="flex flex-col items-center"><div className={`w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center text-2xl md:text-3xl font-black backdrop-blur-lg border transition-colors ${boxBgColor}`}>{timeLeft.minutes.toString().padStart(2, '0')}</div><span className="text-xs font-bold mt-3" style={{ color: isLightBg ? primaryColor : 'white' }}>דקות</span></div>
              <div className="flex flex-col items-center"><div className={`w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center text-2xl md:text-3xl font-black backdrop-blur-lg border transition-colors ${boxBgColor}`}>{timeLeft.hours.toString().padStart(2, '0')}</div><span className="text-xs font-bold mt-3" style={{ color: isLightBg ? primaryColor : 'white' }}>שעות</span></div>
              <div className="flex flex-col items-center"><div className={`w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center text-2xl md:text-3xl font-black backdrop-blur-lg border transition-colors ${boxBgColor}`}>{timeLeft.days.toString().padStart(2, '0')}</div><span className="text-xs font-bold mt-3" style={{ color: isLightBg ? primaryColor : 'white' }}>ימים</span></div>
            </div>
          ) : (
            <div className={`fade-up-item mb-12 p-6 rounded-3xl border backdrop-blur-xl shadow-2xl ${isLightBg ? 'bg-white/60 border-white/50' : 'bg-white/10 border-white/20'}`}>
              <Sparkles className="mx-auto mb-3 text-yellow-400 animate-pulse" size={48} />
              <h2 className={`text-3xl font-black ${headerTextColor} mb-2`}>היום זה קורה!</h2>
