...
const Icebreaker = () => {
  ...
  const checkStatus = async (isMounted = true) => {
    try {
      ...
      const { data: activeMatch } = await supabase
        ...
        .select('id, guest1_id, guest2_id, mission_text, status, photo_url, completed_at, created_at')
        .eq('event_id', eventId)
        .eq('status', 'pending')
  ...
  const handleProfilePhotoUpload = async (e) => {
    ...
    try {
      ...
      await supabase.storage.from('icebreaker-uploads').upload(fileName, compressed, { contentType: 'image/jpeg', upsert: true });
  ...
  const handleProofUpload = async (e) => {
    ...
    try {
      ...
      await supabase.storage.from('icebreaker-uploads').upload(fileName, compressed, { contentType: 'image/jpeg' });
      const { data: { publicUrl } } = supabase.storage.from('icebreaker-uploads').getPublicUrl(fileName);
  ...
  const handleReport = async (matchId) => {
    ...
    try {
      await supabase.from('reports').insert([{
        event_id: eventId, reported_item_id: matchId, item_type: 'icebreaker', reporter_id: guestId
      }]);
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-6 transition-colors duration-1000" style={{ backgroundColor: bgColor }} dir="rtl">
        ...
        <div className="w-full max-w-sm text-center relative z-10 animate-in zoom-in-95 duration-500">
          ...
          <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.07)]">
            <h2 className="text-lg font-bold mb-2 text-slate-800">תמונת זיהוי</h2>
            ...
            >
              {isJoining ? <Loader2 className="animate-spin mx-auto" size={24} /> : 'אני בפנים!'}
            </button>
          </div>
        </div>
      </div>
    ...
    return (
      ...
      >
        <div ref={rouletteRef} className="bg-white/20 p-12 rounded-[3rem] backdrop-blur-md shadow-2xl border border-white/20">
          <Loader2 className="animate-spin mx-auto mb-8" size={72} style={{ color: primaryTextColor }} />
          <h2 className="text-3xl font-black mb-2 drop-shadow-md">מאתר קורבן...</h2>
          <p className="text-lg font-medium mt-3 animate-pulse opacity-70">מגריל משימה חשאית</p>
        </div>
      </div>
    );
  }
  ...
  if (view === 'active_mission' && currentMatch) {
    return (
      <div className="min-h-screen flex flex-col transition-colors duration-1000" style={{ backgroundColor: bgColor }} dir="rtl">
        ...
        >
          <button onClick={() => setView('hub')} className="p-2 bg-black/10 hover:bg-black/20 rounded-full transition-colors">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-6 mission-reveal">
          <p className="text-sm font-bold text-slate-400 mb-4 uppercase tracking-widest">המטרה שלך:</p>
          ...
          >
            {currentMatch.partner?.photo_url
              ? <img src={currentMatch.partner.photo_url} className="w-full h-full object-cover" alt={currentMatch.partner.name} />
          ...
          <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.07)] w-full max-w-md relative">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest mb-3 opacity-50" style={{ color: primaryColor }}>
              פקודת מבצע
            </p>
            <p className="text-2xl font-black text-slate-800 leading-snug">{currentMatch.mission_text}</p>
          </div>
    ...
    <div className="min-h-screen font-sans transition-colors duration-1000 pb-12" style={{ backgroundColor: bgColor }} dir="rtl">
      ...
      >
        ...
        <h1 className="text-2xl font-black flex items-center gap-2" style={{ color: primaryTextColor }}>
          IceBreaker <Zap size={18} style={{ fill: primaryTextColor, opacity: 0.7 }} />
        </h1>
        <div className="w-10 h-10 rounded-[1rem] overflow-hidden bg-black/10 shadow-inner border border-white/10">
          {myProfile?.photo_url
            ...
            : <User size={20} className="m-auto mt-2 opacity-50" style={{ color: primaryTextColor }} />
          }
        </div>
      </div>
      ...
      <div className="px-5 -mt-14 relative z-20 max-w-md mx-auto">
        ...
        >
          {/* accent top bar */}
          <div className="absolute top-0 left-0 w-full h-1.5 rounded-t-[2.5rem]" style={{ backgroundColor: primaryColor }} />
          <div
            className="w-16 h-16 rounded-[1.2rem] flex items-center justify-center mx-auto mb-3"
            style={{ backgroundColor: `${primaryColor}15` }}
          >
            <Zap className="group-hover:animate-bounce" size={30} style={{ color: primaryColor }} />
          </div>
          <h2 className="text-2xl font-black text-slate-800">הגרל משימה חדשה</h2>
          <p className="text-slate-400 font-medium mt-1 text-sm">לחצו כדי לקבל אתגר</p>
        </button>
        ...
        <h3 className="fade-up-item text-lg font-black text-slate-700 mb-4 flex items-center gap-2">
          ...
          {feed.length > 0 && <span className="text-slate-400 font-medium text-sm">({feed.length} משימות)</span>}
        </h3>
        ...
        <div className="space-y-5 pb-8">
          {feed.length === 0 ? (
            <div className="fade-up-item text-center py-14 bg-white rounded-[2rem] shadow-sm border border-slate-100">
              ...
              <p className="text-slate-400 text-sm mt-1">היו הראשונים לבצע משימה!</p>
            </div>
          ) : (
            feed.map(match => (
              <div
              ...
              >
                ...
                <div className="p-5 bg-slate-50/60 border-b border-slate-100">
                  ...
                  <p className="text-base font-black text-slate-800 leading-snug">"{match.mission_text}"</p>
                </div>
                ...
                <div className="p-4 flex items-center justify-between text-xs text-slate-400 font-bold">
                  ...
                  <span dir="ltr">
                    {new Date(match.completed_at).toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
              </div>
            ))
