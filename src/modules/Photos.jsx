...
import { Loader2, ChevronLeft, Camera, Image as ImageIcon, UploadCloud, CheckCircle2, AlertCircle, Heart, Sparkles, X, Download, ChevronRight as ChevRight, ZoomIn } from 'lucide-react';
...
const Photos = () => {
  ...
  const fetchPage = useCallback(async (pageOffset = 0) => {
    if (!eventId) return;
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage
        .from('event-assets')
        .upload(`photos/${fileName}`, compressed, { cacheControl: '3600', upsert: false, contentType: 'image/jpeg' });
      ...
      const newPhoto = { id: Date.now(), guest_name: guestName || 'אורח', image_url: publicUrl, created_at: new Date().toISOString() };
      ...
      <div className="px-5 -mt-8 relative z-20 max-w-md mx-auto">
        ...
        <div className="fade-up-item bg-white p-6 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.07)] border border-slate-100 mb-6 text-center">
          {uploadSuccess ? (
            <div className="py-6 animate-in zoom-in">
              ...
              <p className="text-sm text-slate-500 font-medium">נוספה בהצלחה לאלבום המשותף</p>
            </div>
          ) : (
            <>
              <div className="w-16 h-16 rounded-[1.2rem] flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: `${primaryColor}18` }}>
                <UploadCloud size={28} style={{ color: primaryColor }} />
              </div>
              <h2 className="text-xl font-black text-slate-800 mb-1">צלמו או העלו תמונה</h2>
              <p className="text-slate-400 font-medium text-sm mb-5">
                <span className={`font-bold ${myUploadCount >= MAX_PHOTOS_PER_GUEST ? 'text-rose-500' : 'text-slate-600'}`}>
        ...
        <div className="fade-up-item">
          <h3 className="font-black text-slate-700 mb-4 flex items-center gap-2">
            ...
            {photos.length > 0 && <span className="text-slate-400 font-medium text-sm">({photos.length})</span>}
          </h3>
              ...
              {photos.map((photo, idx) => (
                ...
                >
                  <img
                    ...
                    loading="lazy"
                    style={{ aspectRatio: idx % 3 === 0 ? '4/5' : idx % 3 === 1 ? '1/1' : '4/3' }}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex flex-col justify-end p-3">
                    <div className="flex items-center justify-between">
                      <p className="text-white text-xs font-bold truncate flex items-center gap-1">
                        <Heart size={10} style={{ fill: primaryColor, color: primaryColor }} />
                        {photo.guest_name}
                      </p>
                    ...
                    <p className="text-white/50 text-[10px] mt-0.5">{timeAgo(photo.created_at)}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {hasMore && (
            <div ref={sentinelRef} className="flex justify-center py-6">
              {loadingMore && <Loader2 className="animate-spin text-slate-300" size={24} />}
            </div>
          )}
        </div>
      </div>
      ...
      {lightbox !== null && photos[lightbox] && (
        ...
        >
          ...
          <button
            ...
            onClick={(e) => { e.stopPropagation(); handleDownload(photos[lightbox].image_url); }}
            title="הורד תמונה"
          >
            <Download size={22} />
          </button>
          ...
          <div className="absolute bottom-6 inset-x-0 text-center">
            <p className="text-white/70 text-sm font-bold">{photos[lightbox].guest_name}</p>
